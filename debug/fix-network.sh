...
SSH_OPTS="-o StrictHostKeyChecking=no -o IdentitiesOnly=no"

# 检查 SSH 是否可达
if ! ssh $SSH_OPTS root@127.0.0.1 -p 2222 "true" 2>/dev/null; then
    echo "[fix-network] SSH 不可达 (127.0.0.1:2222)"
    echo "              请先运行: bash debug/start.sh"
    exit 1
fi

echo "[fix-network] 修复网络配置（10.0.2.15/24, gw=10.0.2.2）..."
ssh $SSH_OPTS root@127.0.0.1 -p 2222 "
...
ssh $SSH_OPTS root@127.0.0.1 -p 2222 "rm -f /var/lock/opkg.lock && opkg update 2>&1 | grep -E 'Updated|error|Error'" 2>/dev/null
echo ""
